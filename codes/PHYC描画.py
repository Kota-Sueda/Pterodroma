...
# === フォルダ設定 ===
folder_path = r"D:\Pterodroma\marine_data\phyc_forecast"
save_dir = r"D:\Pterodroma\marine_data\phyc_image"
os.makedirs(save_dir, exist_ok=True)

# === 実行日と深さ設定 ===
start_date = datetime.now().date()
depth_target = 0.4940253794193268  # 表層のSST（m）

# === 可視化処理 ===
for i in range(9):
    date = start_date + timedelta(days=i)
    file_path = f"{folder_path}\\CMEMS_PHYC_{date}.nc"

    try:
        ds = xr.open_dataset(file_path)
        sst = ds['phyc'].sel(depth=depth_target, method='nearest')
        lat = ds['latitude']
        lon = ds['longitude']
        ...
        cbar.set_label('phyc [mmol/m3]')
        plt.title(f'Mole concentration of phytoplankton on {date}', fontsize=14)

        # 保存
        plt.savefig(os.path.join(save_dir, f"PHYC_plot_day{i}_{date}.png"), dpi=150, bbox_inches='tight')
        plt.close()
