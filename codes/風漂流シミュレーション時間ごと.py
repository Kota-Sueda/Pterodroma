...
# 軌跡と出発・終点
ax.plot(lons, lats, color='blue', linewidth=1.5, transform=ccrs.PlateCarree())
ax.plot(lons[0], lats[0], marker='*', color='green', markersize=10, label='Start', transform=ccrs.PlateCarree())
ax.plot(lons[-1], lats[-1], marker='X', color='red', markersize=8, label='End', transform=ccrs.PlateCarree())

plt.title("Passive Drift Simulation (600 hourly steps)\nGlobal L4 Wind", fontsize=14)
plt.legend()
plt.show()
